...
export const StatusModal: React.FC<StatusModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="bg-white border border-slate-200 rounded-2xl w-full max-w-4xl overflow-hidden shadow-2xl flex flex-col max-h-[88vh] animate-scale-up"
      ...
      >
        ...
        <div className="p-4 border-b border-slate-100 bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shrink-0">
          ...
          <div className="flex items-center justify-between sm:justify-end space-x-3 text-[11px] text-slate-500 font-medium shrink-0">
            <span>Showing {totalSubjectsFiltered} of {totalSubjectsAll} subjects</span>
            {semesterGroups.length > 1 && (
              <button
                onClick={toggleExpandAll}
                className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-lg border border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold transition-colors cursor-pointer text-[11px]"
              >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      className="w-full flex items-center justify-between p-3.5 bg-slate-50 hover:bg-indigo-50/40 border-b border-slate-200 cursor-pointer select-none transition-colors"
                    >
                      <div className="flex items-center space-x-3 min-w-0">
                        <span className="px-2.5 py-1 rounded-lg bg-slate-900 text-white font-black text-xs font-mono shrink-0 shadow-2xs">
                          {grp.semNumberDisplay !== 'Unassigned' ? `Sem ${grp.semNumberDisplay}` : 'Unassigned Sem'}
                        </span>
                        <div className="min-w-0">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-[11px] text-slate-500 font-medium truncate">
                              {grp.subjectStatsList.length} Subject{grp.subjectStatsList.length !== 1 ? 's' : ''} Logged
                              {grp.nonCreditCount > 0 && (
                                <span className="ml-1 text-amber-600 font-bold">
                                  ({grp.nonCreditCount} Non-Credit)
                                </span>
                              )}
                            </span>
                          </div>
                          <p className="text-[10px] text-slate-400 font-medium">
                            Pass / Fail Ratio: <strong className="text-emerald-700">{grp.passStudentsCount} Pass</strong> | <strong className="text-red-600">{grp.failStudentsCount} Fail</strong>
                          </p>
                        </div>
                      </div>
                    ...
                    {expanded && (
                      <div>
                          ...
                          <table className="w-full text-left border-collapse">
                            <thead>
                              <tr className="bg-slate-100/70 border-b border-slate-200 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                                <th className="px-4 py-2 min-w-[220px] border-r border-slate-200">Subject Information</th>
                                <th className="px-4 py-2 text-center min-w-[100px] border-r border-slate-200">
                              ...
                              {grp.subjectStatsList.map((stat, idx) => {
                                ...
                                return (
                                  <tr key={idx} className="hover:bg-slate-50/60 transition-colors">
                                    ...
                                    {/* Total Students */}
                                    <td className="px-4 py-2 text-center font-bold text-slate-700 border-r border-slate-100">
                                      {stat.totalStudents}
                                    </td>

                                    {/* Total Pass */}
                                    <td className="px-4 py-2 text-center font-extrabold text-emerald-700 border-r border-slate-100 bg-emerald-50/10">
                                      {stat.totalPass}
                                    </td>

                                    {/* Total Fail */}
                                    <td className="px-4 py-2 text-center font-extrabold text-red-600 border-r border-slate-100 bg-red-50/10">
                                      {stat.totalFail}
                                    </td>
                        ...
                        {grp.semTopStudents.length > 0 && (
                          <div className="p-3 bg-indigo-50/30 border-t border-slate-200">
                            <div className="flex items-center justify-between gap-2 mb-2">
                              <div className="flex items-center space-x-1.5 min-w-0">
                                <Trophy className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                                <span className="text-[11px] font-extrabold text-slate-800 uppercase tracking-tight truncate">
          ...
          {semesterGroups.some((g) => g.semTopStudents.length > 0) && (
            <div className="mt-6 border-t border-slate-200 pt-5 shrink-0 space-y-3.5">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div 
                  className="inline-flex items-center gap-1.5 text-xs text-slate-600 bg-slate-100/90 border border-slate-200/90 px-2.5 py-1 rounded-full font-medium cursor-help shadow-2xs"
                  title="Only students with overall PASS status are eligible for toppers"
                >
                  <Info className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
                  <span>Only students with overall <strong>PASS</strong> status are eligible for toppers</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                {semesterGroups.map((grp) => {
                  ...
                  return (
                    <div key={grp.semKey} className="bg-slate-50/80 border border-slate-200/90 rounded-xl p-3.5 space-y-2.5 shadow-3xs">
                      <div className="flex items-center justify-between border-b border-slate-200/80 pb-2">
                        <div className="flex items-center space-x-2">
                          <span className="px-2 py-0.5 rounded bg-slate-900 text-white font-black text-[10px] font-mono shadow-2xs">
                      ...
                      <div className="space-y-2">
                        {grp.semTopStudents.map((item, idx) => {
                          ...
                          return (
                            <div
                              key={item.student.id || idx}
                              className={`flex items-center justify-between p-2.5 rounded-lg border ${theme.rowBg} transition-colors gap-2 shadow-3xs`}
                            >
                              <div className="flex items-center space-x-2.5 min-w-0">
                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-extrabold border shrink-0 ${theme.badgeBg}`}>
                                  {theme.icon}
                                  {theme.label}
                                </span>
                                <div className="min-w-0">
